...
func TestImageEditsEndpointSpec_ParseBody(t *testing.T) {
	...
	t.Run("rejects JSON body", func(t *testing.T) {
		_, _, _, _, err := spec.ParseBody([]byte(`{"model":"gpt-image-1"}`), false)
		require.ErrorContains(t, err, "expected multipart/form-data")
	})

	t.Run("invalid multipart body", func(t *testing.T) {
		_, _, _, _, err := spec.ParseMultipartBody([]byte("not multipart"), "multipart/form-data; boundary=testboundary", false)
		require.ErrorContains(t, err, "malformed request")
	})
	...
	t.Run("success with model and prompt", func(t *testing.T) {
		...
		model, parsed, stream, mutated, err := spec.ParseMultipartBody(body, "multipart/form-data; boundary=testboundary", false)
		require.NoError(t, err)
		require.Equal(t, "gpt-image-1", model)
